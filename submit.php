<?php
	...
	require_once('include/function.php');
   	...
   	$last = get_last();    // For Form Validification 
   	$wrong_input = 0;
   	for($i = 1;$i <= $last; $i++ )
   	{
      	if(!isset($_POST[$i]) || $_POST[$i]=="")		$wrong_input++; 
   	}
   	//echo $wrong_input;
	if($wrong_input != 0) 
	{
	    ...
	    while($questions_fetch = mysql_fetch_array($questions_query, MYSQL_ASSOC))
        {
			...
			echo "<br />";
         	if($questions_fetch["answer_type"] == 0) 
       		{
       		  	if($_POST[$id]=="") echo '&nbsp &nbsp &nbsp<input type = "textarea" name = "'.$id.'">' ."<a class = 'asterisk'>***</a>";
         		else  echo '&nbsp &nbsp &nbsp<input type = text name = "'.$id.'" placeholder = "'.$_POST[$id].'" value = "'.$_POST[$id].'">';
            	echo "<br /><br />";
         	}
         	else 
			{
